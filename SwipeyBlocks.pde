...
// Remove blocks that have moved off-screen and increment the score
void updateScore() {
...
// Move all the columns and loose blocks
void move() {
  for (Column column : vecOfColumns) {
    column.move();
...
void draw() {
  ...
  if (!gameIsOver()) { //<>//
    ...
    if (timeInCycle > 0 && millis() - timeInCycle > 2000) {
      risingSpeed += .1;
      for (Column column : vecOfColumns) {
